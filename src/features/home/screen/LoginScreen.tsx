...
export const LoginScreen = () => {
  const [step, setStep] = useState(1); // 1: Elegir, 2: Nombre
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [name, setName] = useState('');
  const setProfile = useUserStore((state) => state.setProfile);

  // Progreso de la animación (0 = rejilla, 1 = arriba y grande)
  ...
  return (
    <LinearGradient colors={['#00AEEF', '#FFFFFF']} style={styles.container}>
      ...
      <View style={styles.centerContainer}>
        {AVATARES.map((item) => {
          ...
          const animatedStyle = useAnimatedStyle(() => {
            ...
            // Si no es el seleccionado, desaparece suavemente
            return {
      ...
      {/* ACTO 2: APARECE EL NOMBRE (Solo cuando step es 2) */}
      {step === 2 && (
...
const styles = StyleSheet.create({
  ...
  mainTitle: { fontSize: 30, fontWeight: '900', color: '#FFF', textShadowColor: 'rgba(0,0,0,0.1)', textShadowRadius: 10 },
  centerContainer: {
    flexDirection: 'row',
    flexWrap: 'wrap',
  ...
  circle: { width: 90, height: 90, borderRadius: 45, elevation: 10, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 5 },
  inputContainer: { 
    position: 'absolute', 
    bottom: height * 0.2, 
    width: '100%', 
